...
Both are the same single-file app (`app/index.html`): the working messaging UI with six starter characters, a group chat, on-device memories (localStorage), character creation, simulated voice calls, and themes. Replies come from **your own model server** — a machine on your network running Ollama, llama.cpp, LM Studio or vLLM, configured under Settings → Your server (see [docs/11-model-server.md](docs/11-model-server.md)). Conversations travel only between your device and that machine. Build pipeline: [`android/README.md`](android/README.md) — the APK is built without the Android SDK (blocked network) using Maven Central tooling and a hand-rolled binary-manifest encoder.
